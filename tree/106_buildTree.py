...
def build_tree_by_inorder_and_postorder(inorder: list, postorder: list) -> TreeNode:
    if not inorder or not postorder or len(inorder) != len(postorder):
        return
    i, j = len(postorder) - 1, 0
    root = TreeNode(postorder[i])
    # 将中序遍历列表和后序遍历列表划分为左右子树的中序遍历列表和后序遍历列表
    j = inorder.index(postorder[i])
    # 左子树的中序遍历列表和后序遍历列表
    left_inorder = inorder[:j]
    left_postorder = postorder[:j]
    # 右子树的中序遍历列表和后序遍历列表
    right_inorder = inorder[j + 1:]
    right_postorder = postorder[j:i]
    root.left = build_tree_by_inorder_and_postorder(left_inorder, left_postorder)
    root.right = build_tree_by_inorder_and_postorder(right_inorder, right_postorder)
    return root
...
if __name__ == '__main__':
    ...
    postorder = list(filter(None, print_tree_postorder(tree_root1)))
